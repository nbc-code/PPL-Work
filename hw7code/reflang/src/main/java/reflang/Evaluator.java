...
public class Evaluator implements Visitor<Value> {
	...
	List<Value.RefVal> locs = new ArrayList<>(); // List of freed mem locations for problem 5
	boolean flag = false; // Flag for problem 5
	...
	public Value visit(LetExp e, Env env) { // New for varlang.
		...
		Value retVal = null; //Initialize return value
		...
		for (int index = 0; index < names.size(); index++){
			...
			retVal = heap.deref((RefVal) values.get(index)); // Deref and save return value before freeing anything
			heap.setRefName(names.get(index)); // Set name for freed variable
			heap.free((RefVal) values.get(index)); // Free mem location
			e.body().accept(this, new_env);
		}
	...
	*/
	...
	// Original LetExp, use when testing problem 4. Problem 2 changes LetExp and collides with solution to problem 4.
	@Override
	public Value visit(LetExp e, Env env) { // New for varlang.
		List<String> names = e.names();
		List<Exp> value_exps = e.value_exps();
	...
	// NOTE, make sure to swap out modified LetExp from problem 2 to original LetExp.
	// The modified LetExp from problem 2 collides with solution to problem 4.
	@Override
	public Value visit(AssignExp e, Env env) { // New for reflang
		...
		Value assign_val = null;

		if(rhs_val.getClass().isAssignableFrom(heap.deref(loc).getClass())) { // Check types
			assign_val = heap.setref(loc, rhs_val);
		}
		else
		{
			// Types do not match up, raise dynamic error
			String name = ((VarExp) lhs)._name;
			return new DynamicError("Assigning a value of an incompatible type to the location (set! " + name + " " + rhs_val.tostring() + ")");
		}

		return assign_val;
	}

	// New to RefLang for problem 5
	@Override
	public Value visit(RefHashExp e, Env env) {
		Exp rhs = e.rhs_exp();
		Exp lhs = e.lhs_exp();
		Value.BoolVal ret = null;
		Class type1 = null;
		Class type2 = null;
		Value rhs_val = null;
		...
		try{
			rhs_val = (Value) rhs.accept(this, env);
			loc = (Value.RefVal) lhs.accept(this, env);
			type1 = heap.deref((RefVal) rhs_val).getClass();
			type2 = heap.deref(loc).getClass();
		}
		catch(ClassCastException ex) // Unable to cast for deref, check original expressions
		{
			type1 = rhs.getClass();
			type2 = lhs.getClass();
		}

		if(type1 == type2) { // Type check
	...
	public Value visit(FreeExp e, Env env) { // New for reflang.
		...
		VarExp vexp = (VarExp) value_exp; // Cast to get name of variable
		heap.setRefName(vexp._name); // Set variable name in heap

		for(Value.RefVal v : locs){ // Check mem locations that were already freed
			if(loc == v) { // Match found
				System.out.println("Part 3: Wrong memory location (" + loc.tostring() + "). Already freed.");
				flag = true; // Flag set
				break;
			}
		}

		if(!flag) { // Flag not set, proceed to free mem loc
			locs.add(loc); // Add freed mem loc to list
			heap.free(loc); // Free mem loc
		}
		else {
