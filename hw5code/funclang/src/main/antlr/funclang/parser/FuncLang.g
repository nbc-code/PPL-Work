...
import ListLang; //Import all rules from ListLang grammar.
 ...
 switchexp  returns [SwitchExp ast]
        locals [ArrayList<Exp> case_exps = new ArrayList<Exp>(), ArrayList<Exp> case_bodys = new ArrayList<Exp>()] :
 		'(switch' '(' e0=exp ')'
 		     ( '(' 'case' e=exp body=exp { $case_exps.add($e.ast); $case_bodys.add($body.ast); } ')' )*
 		     '(' 'default' defbody=exp ')'
 		')' { $ast = new SwitchExp($e0.ast, $case_exps, $case_bodys, $defbody.ast); }
 		;
