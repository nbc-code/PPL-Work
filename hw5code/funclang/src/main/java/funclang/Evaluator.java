...
public class Evaluator implements Visitor<Value> {
	...
	public Value visit(LessExp e, Env env) { // New for funclang.
		...
		if(e.first_exp().accept(this, env).getClass() == NumVal.class && e.second_exp().accept(this, env).getClass() == NumVal.class) {
		...
		else if(e.first_exp().accept(this, env).getClass() == PairVal.class && e.second_exp().accept(this, env).getClass() == PairVal.class || e.first_exp().getClass() == ListExp.class && e.second_exp().getClass() == ListExp.class) {
			Value.PairVal first;
			Value.PairVal second;
			try {
				first = (Value.PairVal) e.first_exp().accept(this, env);
			}
			catch (ClassCastException w) {
				first = null;
			}
			try {
				second = (Value.PairVal) e.second_exp().accept(this, env);
			}
			catch (ClassCastException w) {
				second = null;
			}

			ArrayList<NumVal> flist = new ArrayList<>();
			ArrayList<NumVal> slist = new ArrayList<>();

			NumVal f1;
			NumVal s1;

			while(true && first != null) {
				f1 = (NumVal) first._fst;
				flist.add(f1);
				try {
					first = (PairVal) first._snd;
				}
			...
			while(true && second != null) {
				s1 = (NumVal) second._fst;
				slist.add(s1);
				try {
					second = (PairVal) second._snd;
				}
				catch (ClassCastException q) {
					break;
				}
			...
			else if(flist.size() == slist.size()){
				for(int i = 0; i < flist.size(); i++)
				{
					if(flist.get(i).v() >= slist.get(i).v())
		...
		else if (e.first_exp().accept(this, env).getClass() == StringVal.class && e.second_exp().accept(this, env).getClass() == StringVal.class) {
			Value.StringVal first = (Value.StringVal) e.first_exp().accept(this, env);
			Value.StringVal second = (Value.StringVal) e.second_exp().accept(this, env);

			int length1 = first.v().length();
			int length2 = second.v().length();

			if (length1 < length2) {
				return new Value.BoolVal(true);
			} else if (length1 == length2) {
				for (int i = 0; i < length1; i++) {
					if (first.v().charAt(i) >= second.v().charAt(i)) {
						return new Value.BoolVal(false);
					}
				}
	...
	public Value visit(EqualExp e, Env env) { // New for funclang.
		...
		if(e.first_exp().accept(this, env).getClass() == NumVal.class && e.second_exp().accept(this, env).getClass() == NumVal.class) {
			...
			if(flist.size() < slist.size() || flist.size() > slist.size())
				return new Value.BoolVal(false);
			else {
				for(int i = 0; i < flist.size(); i++) {
					if(flist.get(i).v() != slist.get(i).v()){
		...
		else if (e.first_exp().getClass() == BoolExp.class && e.second_exp().getClass() == BoolExp.class){
			Value.BoolVal first = (Value.BoolVal) e.first_exp().accept(this, env);
			Value.BoolVal second = (Value.BoolVal) e.second_exp().accept(this, env);
			return new Value.BoolVal(first.v() && second.v());
		}
		return new Value.BoolVal(false);
	}
	...
	public Value visit(SwitchExp e, Env env) {
		Value.NumVal e0 = (Value.NumVal) e.e0().accept(this, env);
		Value.NumVal defbody = (Value.NumVal) e.defbody().accept(this, env);
		ArrayList<NumVal> elems = new ArrayList<>();
		ArrayList<NumVal> bodys = new ArrayList<>();

		for(Exp exp : e.elems()){
			elems.add((NumVal) exp.accept(this, env));
		}
		for(Exp exp : e.bodys()){
			bodys.add((NumVal) exp.accept(this, env));
		}

		for(int i = 0; i < elems.size(); i++) {
			if(elems.get(i).v() == e0.v()){
				return new Value.NumVal(bodys.get(i).v());
			}
		}

		return new Value.NumVal(defbody.v());
	}
